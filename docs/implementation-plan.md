...
- 一般的な進行知識として、別々の対象への白結果は結果矛盾ではないこと、白結果だけの占い師候補2人を即座に処刑対象へ限定しないこと、黒結果では黒を出された本人と占い師候補を比較すること、霊媒師候補2人では順番に処刑する進行を検討することをv3へ与える。
...
- 真占い師は黒結果、既存対抗、日数、seedで選ばれたCO時機によって公開を早め、遅くとも2日目には名乗る。真霊媒師は公開処刑結果と対抗状況を見て判断し、遅くとも3日目には名乗る。
- 狂人はseedと人格差から高めだが固定ではない確率で占い師または霊媒師を騙る。人狼2名のうちseedで選ばれた1名は状況依存で騙り、非担当の人狼も自分への黒結果などで緊急対抗し得る。全員潜伏、狂人対抗、人狼対抗をいずれも再現可能にする。
...
- 新規試合は第0夜、1日目の順に開始を画面全面で示す。1日目の演出は第0夜の占い結果確認後、最初の発言より前に表示し、全画面演出中はVOICEと発言表示を待機させる。以後も夜明けの日数、投票開始、開票、処刑を2.4〜3.6秒の切替演出で示し、連続する演出間には0.6秒の間を設ける。襲撃犠牲者と処刑者は名前を表示し、それぞれ専用のCC0効果音を鳴らす。効果音は独立してON/OFFでき、再読込・再接続・視点切替で過去イベントを重複再生しない。
...
- 真占い師単独CO、狂人対抗、人狼対抗、騙り役全潜伏、狂人の狼黒誤爆／白囲い、複数日の偽結果履歴をfixture seedで再現できる。
