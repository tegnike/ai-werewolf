...
- 後続話者には議論台帳と不足している貢献候補を渡し、同じ質問の反復より、独自の灰評価、弁明、役職候補の扱い、処刑方針、投票予定を優先させる。候補は台詞や結論を固定する台本にはしない。
...
- 2人以上から疑いを向けられ、当日まだ1回しか話していない人物には、必須の役職主張より後、未発言者のseed選択より前に1回だけ弁明機会を与える。明示的な返答要求による次話者指定はこれより優先する。
...
新規試合の `match_created` は `rules.claims = 'v1'` と `rules.discussion = 'v3'` を持つ。`discussion_speech` は従来の台詞、宛先、返答要求、`claim` に加えて、v3では公開発言の自己分類である `structure` を保存する。`claim` は `null` または `claimedRole` と結果配列だけを含む。`stage = 'opening'` はその人物の当日1回目、`stage = 'free'` は2回目の発言を表し、全体の固定一巡は表さない。`rules.discussion = 'v2'` の保存済み試合はv2の発言schema、プロンプト、キュー、payloadを変更せず再開する。`rules.discussion` markerのない旧試合は従来の固定一巡経路で再開する。公開claim ledger、観戦UI、リプレイは保存済みの `discussion_speech.claim` だけから導出し、真役職・真結果・非公開方針を参照しない。v3の `structure` は当面、公開観戦画面へ表示せず、議論制御と受け入れ評価にだけ使う。
...
勝率やゲームバランスを測るために実AIを複数試合実行しない。議論品質の3試合受け入れは `ALLOW_REAL_AI=1` と `npm run accept:day1` の二重条件で、1日目の処刑後に停止し、試合ID、発言、構造化評価、消費呼び出し数を保存する。問題があればまず保存ログとMockAIで調査し、配役・ルール変更はマスターの承認なしに行わない。
