...
- 新規試合の `discussion v3` では、各発言が本文と一致する `primaryAct`、質問または回答の話題、暫定的な疑い先と根拠分類、投票予定を構造化して返す。これは発言者の公開した立場を記録するもので、真の役職や内部推論ではない。
...
- 必須主張がschema、本文、認可内容と一致しない場合は有界回数だけ同一モデルで再試行し、直らなければ試合を明示停止する。主張を黙って削る、ランダム行動へ置換する、別モデルへフォールバックする処理は禁止する。
...
2. 生存人狼が非公開会話を2周行う。生存人狼が1名だけの場合も同じ発言機会を持つが、会話modeを `monologue` とし、死亡した仲間への相談や返答要求ではなく、自分の読みと襲撃方針を口にする独り言として生成する。構造化応答でも `addressedTo=null`、`requestsReply=false` を強制する。
3. 各人狼が襲撃候補を選ぶ。
4. 生存占い師が占い対象を選ぶ。
5. 生存狩人が護衛対象を選ぶ。
6. 夜行動を同時解決する。
...
- 新規試合は第0夜、1日目の順に開始を画面全面で示す。1日目の演出は第0夜の占い結果確認後、最初の発言より前に表示し、全画面演出中はVOICEと発言表示を待機させる。以後も夜明けの日数、開票、処刑を2.4〜3.6秒の切替演出で示す。通常の連続する演出間には0.6秒、開票から次の演出までには投票結果を読むため5秒の間を設ける。投票開始は全画面演出にせず、画面上のフェーズ表示だけで示す。襲撃犠牲者と処刑者は名前を表示し、それぞれ専用のCC0効果音を鳴らす。効果音は独立してON/OFFと音量調整ができ、設定は再読込後も維持する。再接続・視点切替で過去イベントを重複再生しない。
