...
新規試合の `match_created` は `rules.claims = 'v1'` と `rules.discussion = 'v2'` を持つ。`discussion_speech` は従来の台詞、宛先、返答要求に加えて `claim` を持ち、`claim` は `null` または `claimedRole` と結果配列だけを含む。`stage = 'opening'` はその人物の当日1回目、`stage = 'free'` は2回目の発言を表し、全体の固定一巡は表さない。`rules.discussion` markerのない旧試合は従来の固定一巡経路で再開し、保存済みイベントとの照合を壊さない。公開claim ledger、観戦UI、リプレイは保存済みの `discussion_speech.claim` だけから導出し、真役職・真結果・非公開方針を参照しない。
