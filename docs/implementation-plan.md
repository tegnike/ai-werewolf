...
- AIは `Agent 1` から `Agent 9` の匿名プレイヤーとして参加する。
...
- プレイヤー名は `Agent 1`〜`Agent 9`。
- 座席番号と名前は一致させる。
- 独自の人物設定や職業、演技口調を与えない。
- 全員に同一の推理能力・文章制約を与え、役職と見えている情報だけを変える。
- UI上は色と抽象AIアバターで識別するが、性能差を示唆しない。
...
- 中央: Agent 1〜9のカード。座席、生死、最新発言、GM視点の役職バッジ。
...
- UIのAgent 1〜9アバターは色違いの独自SVGとし、人物設定を付けない。
