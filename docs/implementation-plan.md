...
- 生存者全員が最低1回話し、1人の発言は1日最大2回とする。9人生存時は合計14発言未満で終了せず、18発言で必ず終了する。
...
- 既存対抗への発言が必要な場合は動的キューの次話者候補へ直接入れる。intent pollを追加せず、1日の総発言数、1人2回、1日2 pollの上限は維持する。
...
新規試合の `match_created` は `rules.claims = 'v1'` と `rules.discussion = 'v3'` を持つ。`discussion_speech` は従来の台詞、宛先、返答要求、`claim` に加えて、v3では公開発言の自己分類である `structure` を保存し、変更のない投票予定再宣言を主要貢献から格下げした場合だけ `contributionDemoted=true` を保存する。`claim` は `null` または `claimedRole` と結果配列だけを含む。`stage = 'opening'` はその人物の当日1回目、`stage = 'free'` は2回目の発言を表し、全体の固定一巡は表さない。`rules.discussion = 'v2'` の保存済み試合はv2の発言schema、プロンプト、キュー、payloadを変更せず再開する。`rules.discussion` markerのない旧試合は従来の固定一巡経路で再開する。公開claim ledger、観戦UI、リプレイは保存済みの `discussion_speech.claim` だけから導出し、真役職・真結果・非公開方針を参照しない。v3の `structure` と `contributionDemoted` は当面、公開観戦画面へ表示せず、議論制御と受け入れ評価にだけ使う。
...
- 9人生存時は公開発言14〜18回、8人以下は人数比例の範囲とし、生存者全員が最低1回、1人最大2回話す。発言希望確認は1日最大2回・各回最大4人とする。
- 投票の同時性、決選、再同票時処刑なし。
- 人狼襲撃不一致時の追加決定。
- 占い、霊媒、護衛、襲撃合法対象。
- 連続護衛禁止。
...
勝率やゲームバランスを測るために実AIを複数試合実行しない。議論品質の3試合受け入れは `ALLOW_REAL_AI=1` と `npm run accept:day1` の二重条件で、1日目の処刑後に停止し、試合ID、発言、構造化評価、消費呼び出し数を保存する。投票予定が4人以上ある場合は、複数の投票先が残るか、最多予定先に公開された `人狼` 結果があることを候補競合条件とする。3人目の同一予定以後は同じ予定の追加宣言を0件とし、後続発言の5分の3以上が対立候補の根拠付き検討、質問・回答、盤面整理、弁明、役職主張のいずれかを増やすことを求める。同じ対立候補を複数ターンで比較しても、単なる勘ではなく反応や発言を更新していれば貢献として数える。5分の3は、旧問題試合の4/7を不合格に保ちつつ、質問へ直接答えて判断を更新する自然な進行を合格にする実AI試験結果から較正した。発言時点の投票予定と最終票の一致率は観測値として保存するが、後続情報を踏まえた独立再評価を妨げるため合否条件にはしない。同一話者の再宣言格下げ数も観測値として報告するが、それ自体を合否条件にしない。問題があればまず保存ログとMockAIで調査し、配役・ルール変更はマスターの承認なしに行わない。
