...
- 開始発言中に、すでに発言を終えた人物へ返答要求が出た場合、その人物は自由討論まで返答不能な「返答待ち」として後続AIへ構造化して渡す。後続AIは同じ返答要求を重ねず、返答がないことを無視・逃避・拒否と評価しない。対抗COの対象だけは返答要求先の除外対象にしない。
...
- 既存対抗への発言が必要な場合は自由討論の次話者候補へ直接入れる。intent pollを追加せず、自由討論総数、1人2回、1日2 pollの上限は維持する。
...
新規試合の `match_created` は `rules.claims = 'v1'` を持つ。`discussion_speech` は従来の台詞、宛先、返答要求に加えて `claim` を持ち、`claim` は `null` または `claimedRole` と結果配列だけを含む。旧試合はmarker不在なら従来経路で再開し、保存済みイベントとの照合を壊さない。公開claim ledger、観戦UI、リプレイは保存済みの `discussion_speech.claim` だけから導出し、真役職・真結果・非公開方針を参照しない。
