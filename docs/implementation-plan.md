...
2. 勝敗判定。
3. 生存者全員が順番に開始発言を1回行い、その後は動的な話者選択による自由討論を行う。
4. 生存者全員が同時投票する。
5. 記名投票結果を公開する。
6. 最多得票者を処刑する。
...
- 最初に生存者全員が開始発言を1回ずつ行う。開始発言は座席順で、毎日開始座席を1つずつずらす。
- 開始発言中は自由な割り込みを行わない。AIには、質問への即時応答は起きず、相手の開始発言または一巡後の自由討論まで待つことを明示する。後の順番の人は、それ以前に自分へ向けられた発言へ自分の開始発言内で応じてよい。
- 開始発言後は、生存者数と同数を上限とする自由討論枠を設ける。1人の自由討論発言は最大2回とし、発言回数には個人差を許す。
- 発言結果は本文に加えて、実際に話を向けた相手と返答要求の有無を返す。返答を求められた生存者を次の話者候補として優先する。
- 発言の連鎖が止まったとき、1日最大2回、各回最大4人へ非公開の発言希望確認を行う。発言希望は強さ、動機、対象だけをstrict schemaで返し、台詞や長い思考過程を含めない。
- 発言希望確認は、台詞生成用の話し方・呼称・台詞例を省いた短いプロンプトと、直近30件の公開履歴だけを使い、入力トークンを抑える。
- 発言希望の候補は、返答を求められている人、まだ確認されていない人、発言回数が少ない人を優先し、同点はseedから決定論的に選ぶ。
- 発言希望がない、2回の確認を使い切る、または自由討論枠を使い切ると投票へ進む。開始発言と自由討論を合わせた公開発言数は、生存者数の2倍を超えない。
...
- 中央: 固有名を持つ9人のカード。座席、生死、死亡日と死因、最新発言、直近の投票先、GM視点と終了後の役職バッジ。カード一覧の下に、発言中のキャラクター画像、名前、セリフを収まる高さで大きく表示し、pause中も同じ発言を保持する。発言ステージと投票結果パネルは同時に表示しない。
- 右: 日区切りと重要イベントの視覚的階層を持つ時系列ログ。発言、記名投票、処刑、夜明け、GM秘密、終了後に開示された秘密を区別する。
...
- 人狼同士だけが互いを知る。
- 第0夜に襲撃・護衛・霊媒がない。
- 昼の開始発言一巡、開始座席ローテーション、自由討論の動的話者選択。
- 発言希望確認は1日最大2回・各回最大4人、公開発言は生存者数の2倍以内、自由討論は1人最大2発言。
- 投票の同時性、決選、再同票時処刑なし。
- 人狼襲撃不一致時の追加決定。
- 占い、霊媒、護衛、襲撃合法対象。
- 連続護衛禁止。
