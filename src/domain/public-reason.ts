...
function actorNames(actorSeat: SeatId): string[] {
  const fullName = agentNameForSeat(actorSeat);
  return [fullName, ...fullName.split(/\s+/)].filter(Boolean);
}

function otherReferenceTerms(actorSeat: SeatId): string[] {
  const addressed = Object.values(addressBookForSeat(actorSeat)).filter((term): term is string => Boolean(term));
  const fullNames = SEATS.filter((seat) => seat !== actorSeat).map(agentNameForSeat);
  return [...new Set([...addressed, ...fullNames])];
}

function mentionsOtherPlayer(reason: string, actorSeat: SeatId): boolean {
  return otherReferenceTerms(actorSeat).some((term) => reason.includes(term));
}

function attributesAbilityToOther(reason: string, actorSeat: SeatId): boolean {
  return otherReferenceTerms(actorSeat).some((term) => {
    const escaped = escapePattern(term);
    return new RegExp(`${escaped}.{0,16}(?:が|の|による).{0,20}(?:占|霊媒|判定|結果)`).test(reason);
  });
}

function unannouncedRoleClaim(reason: string, actorSeat: SeatId, claimLedger: ClaimLedger): boolean {
  const priorRole = claimLedger.find((entry) => entry.seat === actorSeat)?.claimedRole;
  const ownNames = actorNames(actorSeat).map(escapePattern).join('|');
  for (const [role, label] of [['seer', ROLE_LABELS[0]], ['medium', ROLE_LABELS[1]]] as const) {
    ...
    if (reason.split(/[。！？\n]/).some((sentence) =>
      subjectlessClaim.test(sentence.trim()) && !mentionsOtherPlayer(sentence, actorSeat))) return true;
  }
  return false;
}

function unstructuredAbilityResult(reason: string, actorSeat: SeatId): boolean {
  const ownNames = actorNames(actorSeat).map(escapePattern).join('|');
  ...
  return abilityResult.test(reason) && !attributesAbilityToOther(reason, actorSeat);
}
...
export function sanitizePublicVoteReason(
  ...
  const sanitized = unannouncedRoleClaim(reason, actorSeat, claimLedger) ||
    unstructuredAbilityResult(reason, actorSeat);
  return sanitized
