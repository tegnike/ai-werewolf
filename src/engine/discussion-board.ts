...
export function discussionAgenda(
  ...
  if (spoken.size === 0) agenda.push(day === 1
    ? '公開材料がなければ根拠のない疑い先や投票先を無理に作らず、役職を名乗る時期、今日の処刑方針、公開発言後に比べる点を話す。0日目の占い先を選んだ理由は質問・評価しない'
    : '公開済みの役職情報、自分がこの発言で公開する能力結果、前日までの発言がなければ、根拠のない疑い先や投票先を無理に作らず、確認したい論点・質問・判断基準を示す');
  if (spoken.size > 0 && materialPhase === 'scarce') {
    agenda.push(day === 1
      ? '公開材料がまだ少ない。疑い先を無理に作らず、公開後の発言・反応への質問、判断基準、役職の登場時機への意見、または再判断条件つきの保留を示す。0日目の占い先を選んだ理由は質問・評価しない'
      : '公開材料がまだ少ない。疑い先を無理に作らず、宛先つき質問、判断基準、役職の登場時機への意見、または再判断条件つきの保留を一つ示せば十分');
  }
