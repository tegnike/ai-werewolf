...
export default defineConfig({
  ...
  fullyParallel: false,
  workers: 1,
  webServer: { command: `PORT=${port} DATABASE_PATH="${databasePath}" NEXT_DIST_DIR="${distDir}" AI_PROVIDER=mock ALLOW_REAL_AI=0 node scripts/dev.mjs -p ${port}`, port, reuseExistingServer: false, timeout: 120_000 },
